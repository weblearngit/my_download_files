...
使用方法:
    python scripts/run_bfusd_account.py --account-id account1 --api-key YOUR_KEY --api-secret YOUR_SECRET
    或使用环境变量:
        export ACCOUNT_ID=account1
        export BINANCE_API_KEY=YOUR_KEY
        export BINANCE_API_SECRET=YOUR_SECRET
        python scripts/run_bfusd_account.py

配置到crontab示例:
    # 每小时的 01分执行
    1 * * * * /usr/bin/python3 /path/to/project/scripts/run_bfusd_account.py --account-id account1 --api-key KEY --api-secret SECRET --proxy http://xxx.com --per-round-amount 20 --target-amount 1000 >> /var/log/bfusd_account1.log 2>&1
...
from mybian.app.bfusd_service import get_bfusd_service
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=True,
        help='是否必须盈利（仅在有免费额度时生效，bfusd/USDT价格必须<0.999），默认True'
    )
...
def main():
    ...
    lock_file_path = lock_dir / f"bfusd_{args.account_id}.lock"

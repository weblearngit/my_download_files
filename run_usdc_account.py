...
使用方法:
    python scripts/run_usdc_account.py --account-id account1 --api-key YOUR_KEY --api-secret YOUR_SECRET
    或使用环境变量:
        export ACCOUNT_ID=account1
        export BINANCE_API_KEY=YOUR_KEY
        export BINANCE_API_SECRET=YOUR_SECRET
        python scripts/run_usdc_account.py

配置到crontab示例:
    # 每小时的 01分执行
    1 * * * * /usr/bin/python3 /path/to/project/scripts/run_usdc_account.py --account-id account1 --api-key KEY --api-secret SECRET --proxy http://xxx.com --per-round-amount 11 --target-amount 251 >> /var/log/usdc_account1.log 2>&1
...
from mybian.app.usdc_service import get_usdc_service
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help='目标操作金额（USDC），如果达到则停止，默认不限制'
    )
...
def main():
    ...
    lock_file_path = lock_dir / f"usdc_{args.account_id}.lock"
    ...
    try:
        with LockFile(lock_file_path, args.account_id):
            ...
            logger.info(f"总操作数量: {result['total_operated_usdc']:.2f} USDC")
            logger.info(f"总成本: {result['total_cost']:.4f} USDT")
            logger.info(f"总盈亏: {result['total_profit']:.4f} USDT")
            if result['total_operated_usdc'] > 0:
